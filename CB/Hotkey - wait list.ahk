...
Hotkey_AddCB(ControlOption, Name, Option = "", Hotkey = "", Func = "", BindString = "", ByRef hCB = "") { 
	Static IsStart
	Local k, v, g, g1, i, hEdit, arr, fn, hDL, Write, hGui, hDummy
	If (Name + 0 != "") {
	...
	If !Hotkey_Arr("Focus")[hGui := DllCall("GetParent", Ptr, hCB)] {
		Gui, %GuiName%Add, Text, wp hp xp yp Hidden hwndhDummy 
		Hotkey_Arr("Focus")[hGui] := hDummy
	}
	...
	Hotkey_CBArr(hEdit, {hCB:hCB, hDL:hDL, name:Name, arr:arr,func:Func})
	Hotkey_CBArr(Name, hEdit), Hotkey_CBArr(hCB, hEdit), Hotkey_CBArr(hDL, hEdit)
	...
	GuiControl, +g, % hCB, % fn
	If !IsStart
		OnMessage(0x0111, Func("Hotkey_WM_COMMAND"))
	IsStart := 1
	Hotkey_Start() 
}  
...
Hotkey_CBCheck(hEdit) { 
	...
	{
		SendMessage, 0xC, 0, "" Hotkey_Arr("Empty"), , ahk_id %hEdit%
		Hotkey_Value(hEdit, ""), Hotkey_Blink(Hotkey_CBArr(hEdit).hCB)
	} 
	If (func := Hotkey_CBArr(hEdit).func) != ""
		Func(func).Call(hEdit)   
}

Hotkey_WM_COMMAND(wParam, lParam) {
	Static CBN_DROPDOWN := 7, CBN_CLOSEUP := 8 ; msdn.microsoft.com/en-us/library/bb775810(v=vs.85).aspx
	If (wParam >> 16 = CBN_DROPDOWN) && Hotkey_CBArr(lParam)
	{ 
		Hotkey_Arr("CBWait", Hotkey_CBArr(Hotkey_CBArr(lParam)).hDL)
		; SetTimer, Hotkey_CBWait, -50
	} 
	Else If (wParam >> 16 = CBN_CLOSEUP) && Hotkey_CBArr(lParam)
	{ 
		Hotkey_Arr("CBWait", 0), Hotkey_IsRegFocus()
	}
	; ToolTip % wParam >> 16
}
...
Hotkey_CBWait() {
	Local hwnd
	hwnd := Hotkey_Arr("CBWait")
	While DllCall("IsWindowVisible", "Ptr", hwnd)
		Sleep 10
	Hotkey_Arr("CBWait", 0)
	Hotkey_IsRegFocus()
}
...
Hotkey_CBSelect(hEdit, hCB) {
	...
	Count := ErrorLevel
	ToolTip % Pos "`n"  Count "`n " A_GuiEvent "`n" 
...
Hotkey_InitHotkeys(Option = 1) {
	...
	Loop, Parse, nmMouse, |
		Hotkey, % A_LoopField, Hotkey_MView, % Option
...
Hotkey_Arr(P*) {
	Static Arr := {Empty:"Нет", CBAddStr:"    Добавить хоткей", AllHotkeys:{}, BindString:{}, Focus:{}, User:{}}
	Return P.MaxIndex() = 1 ? Arr[P[1]] : P.MaxIndex() = 2 ? (Arr[P[1]] := P[2]) : !P.MaxIndex() ? Arr : Arr.Delete(P[1])
}
...
Hotkey_Delete(Name, Destroy = 1) {
	...
	If IsCB 
		hCB := Hotkey_CBArr(Hwnd).hCB, Hotkey_CBArr(hCB, 0, 0), Hotkey_CBArr(Hotkey_CBArr(Hwnd).hDL, 0, 0)
		, Hotkey_CBArr(Hwnd, 0, 0), Hotkey_CBArr(Name, 0, 0), Hwnd := hCB
	If Destroy
...
Hotkey_KillFocus(Name) { 
	Local hwnd
	hwnd := Hotkey_ID(Name) 
	If InStr(Hotkey_Options(hwnd), "C")
		hwnd := Hotkey_CBArr(hwnd).hCB 
	ControlFocus, , % "ahk_id" Hotkey_Arr("Focus")[DllCall("GetParent", Ptr, hwnd)]
}
...
Hotkey_BanKey("WheelDown|WheelUp", "MyHotkey3")
#IF !GuiShow
#IF
GuiShow := 0
Gui, Font, s12
...
Hotkey_AddCB("wp y+10 r10", "MyHotkey3", "KMLRJG1", Hotkey_Read("MyHotkey3"))
Hotkey_AddCB("wp y+10 r6", "MyHotkey4", "KMJG1", Hotkey_Read("MyHotkey4"))
Gui, Add, ComboBox, wp y+10 r6, 1||2
Gui, Add, Text, xp w33 y+10 border, text
Gui, Add, Edit, x+10 yp
Hotkey_KillFocus("MyHotkey4")
