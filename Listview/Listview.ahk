...
Loop 52
	s .= A_Space
Gui, -DPIScale
Gui, Add, Listview, x300 y100 w490 h480 cBlue AltSubmit gListview vListview hwndhLW -ReadOnly NoSortHdr Grid -Multi -LV0x10, Хоткей%s%|Действие     |Комментарий   ;	-LV0x10
Gui, Show
Loop 4
{
	Name := "Action" A_Index
	HK := Hotkey_Register(Name, , "KMLRJG1", "*", "Save")
	LV_Add( "", HK, Name)
} 
LV_ModifyCol(1, "Center")
LV_SetColumnOrder(hLW, [2,1,3])  
Sleep 101
Gui, Show
Return
	
LV_SetColumnOrder(hwnd, Array) { 
	Static LVM_FIRST := 0x1000, LVM_REDRAWITEMS := 21, LVM_SETCOLUMNORDERARRAY := 58
    VarSetCapacity(lpiArray, Array.Count() * 4, 0)  
	GuiControl, -Redraw, %hwnd%
	for i, v in Array
        NumPut(v - 1, lpiArray, (i - 1) * 4, "UInt")
    SendMessage, LVM_FIRST + LVM_SETCOLUMNORDERARRAY, Array.Count(), &lpiArray, , ahk_id %hwnd% 
    SendMessage, LVM_FIRST + LVM_REDRAWITEMS, 0, Array.Count() - 1, , ahk_id %hwnd%  
	VarSetCapacity(lpiArray, 0)
	GuiControl, +Redraw, %hwnd% 
}
...
Listview:
	If (A_GuiEvent == "E")
	{
		Name := ArrLVActions[LV_GetNext(, "Focused")]
		Hotkey_Register(Name, Listview_GetEdit(hLW), "#", "#", "#")
		
		; ControlGetFocus, ControlNN, A
		; ControlGet, hFocus, Hwnd, , %ControlNN%, A
		; ToolTip % hFocus "`n" Format("0x{1:x}",Listview_GetEdit(hLW))
	}  
	Return
 
Save(Name) {
	ToolTip % Hotkey_Write(Name) "`n" Name "`n" A_TickCount, -33, -33, 19
...
Listview_GetEdit(hLW) {
	Child := DllCall("GetWindow", Ptr, hLW, UInt, 5, Ptr)
	Loop
	{
		WinGetClass, CtrlClass, ahk_id %Child% 
		If CtrlClass = Edit
			Return Child
		If !Child := DllCall("GetWindow", Ptr, Child, UInt, 2, Ptr)
			Return
	}
...
Hotkey_Add(ControlOption, Name, Option = "", Hotkey = "", Func = "", BindString = "", ByRef hEdit = "") {
	...
	Hotkey_Arr("Function")[Name] := Func(Func).Bind(Name, hEdit)
	Hotkey_Start()
}

Hotkey_Register(Name, hEdit = 0, Option = "", Hotkey = "", Func = "", BindString = "") {
	Local M, M1, M2, PrHwnd, PrOptions, Write
	Static fnames := {}
	If (Name + 0 != "") {
		MsgBox, 4112, Hotkey add error, % "Name '" Name "' can not be a number, or already exists.`nPress ok to exit app."
		ExitApp
	} 
	PrHwnd := Hotkey_ID(Name)  ;	Delete
	Hotkey_Value(PrHwnd, "", "")
	PrOptions :=  Hotkey_Options(PrHwnd, "", "")
	Hotkey_ID(PrHwnd, "", "")
	
	If !hEdit
		hEdit := -A_TickCount
	Else 
		Control, Style, +0x800, , ahk_id %hEdit%  ;	+ReadOnly
	If Hotkey = *
		Hotkey := Hotkey_Read(Name)
	Else If Hotkey ~= "^:"
		Hotkey := SubStr(Hotkey, 2), Write := 1
	...
	Hotkey_Arr("BindString")[Name] := BindString
	Hotkey_Arr("AllHotkeys")[Name] := hEdit
	Func := (Func = "#") ? fnames[Name] : Func
	fnames[Name] := Func
	Hotkey_Arr("Function")[Name] := Func(Func).Bind(Name, hEdit)
	Hotkey_Start()
	Return Hotkey_HKToStr(Hotkey)
}
...
Hotkey_Main(Param1, Param2 = "") {
	...
	; _________________________________________________ InitHotkeys _________________________________________________
...
Hotkey_SetWinEventHook(eventMin, eventMax, hmodWinEventProc, lpfnWinEventProc, idProcess, idThread, dwFlags) {
	...
	; _________________________________________________ Get and set _________________________________________________

Hotkey_Arr(P*) {
	Static Arr := {Empty:"Нет", AllHotkeys:{}, Function:{}, BindString:{}, Focus:{}, User:{}}
...
Hotkey_Delete(Name, Destroy = 1) {
	...
	Hotkey_Arr("Function").Delete(Name)
	ControlGetFocus, ControlNN, A
	ControlGet, hFocus, Hwnd, , %ControlNN%, A
...
Hotkey_SetText(hwnd, Text, HK) {
	...
	PostMessage, 0x00B1, -2, -2, , ahk_id %hwnd%		;	EM_SETSEL
	funcobj := Hotkey_Arr("Function")[Hotkey_ID(hwnd)]
	Try SetTimer, % funcobj, -10
}
...
Hotkey_Write(Name, Section = "", FilePath = "") {
	...
	; _________________________________________________ BanKey _________________________________________________
...
Hotkey_IsBan(HK, hwnd) {
	...
	; _________________________________________________ Control _________________________________________________
...
Hotkey_Hide(Name, Hide = 1) {
	...
	; _________________________________________________ Group _________________________________________________
